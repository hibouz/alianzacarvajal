...
//			add_action( 'etheme_last_style', array( $this, 'et_multiple_single_product' ), 100 );
		...
		ob_start();
			...
			switch ( $this->post['template'] ) {
				...
				case 'multiple-headers':
					require_once( ET_CORE_DIR . 'app/models/customizer/builder/template-parts/conditions/multiple-headers.php' );
					break;
	...
	public function generate_header_builder_style( $requested = 'all' ){
		...
		if ( '' == $requested || is_a( $requested, 'WP_Customize_Manager' ) ) {
			$requested = 'all';
		}
				 	...
				 	if ( ! empty( $value['font-family'] ) ) {
				 		...
				 		if ( is_array( $etheme_fonts ) && array_search( str_replace('"', '', $value['font-family']), array_column( $etheme_fonts, 'name') ) === false ) {
				 			...
				 			if ( ! in_array( $value['font-family'], array( 'initial', 'inherit' ) ) ) {
				 				// Get header builder fonts
				 				$header_builder_fonts = get_option( 'et_header_builder_fonts', array() );
				 				// Push new font to old ones
				 				array_push( $header_builder_fonts, array('family' => $family, 'name' => str_replace( ' ', '', $value['font-family'] ) ) );
				 				// Update newly added fonts
				 				update_option( 'et_header_builder_fonts', $header_builder_fonts );
				 			}
				 		}
				 	}
				 }
	...
	public function et_ajax_multiple(){
		...
		switch ( $this->post['type'] ) {
			...
			case 'save':
				...
				if ( isset( $this->post['options']['search_placeholder_et-desktop'] ) ) {
			...
			case 'old':
				...
				$this->post['options']['html_block1'] =  isset( $this->post['options']['html_block1'] ) ? base64_decode( $this->post['options']['html_block1'] ) : '';
				$this->post['options']['html_block2'] =  isset( $this->post['options']['html_block2'] ) ? base64_decode( $this->post['options']['html_block2'] ) : '';
				$this->post['options']['html_block3'] =  isset( $this->post['options']['html_block3'] ) ? base64_decode( $this->post['options']['html_block3'] ) : '';
