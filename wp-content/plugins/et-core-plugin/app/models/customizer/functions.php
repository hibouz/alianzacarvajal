...
function etheme_vertical_header() {
	if ( !get_query_var('is_mobile', false) && get_theme_mod( 'header_vertical_et-desktop', '0' ) ) {
            ...
            <div class="cart-popup-footer">
                <a href="<?php echo esc_url( wc_get_cart_url() ); ?>"
                   class="btn-view-cart wc-forward"><?php esc_html_e( 'Shopping cart ', 'xstore-core' ); ?>
                    (<?php echo $count; ?>)</a>
                <div class="cart-widget-subtotal woocommerce-mini-cart__total total" data-amount="<?php echo $amount; ?>">
					<?php
					/**
...
if ( ! function_exists( 'etheme_mini_wishlist_content' ) ) {
	...
	function etheme_mini_wishlist_content() {
		...
		$limit = get_theme_mod('mini-cart-items-count', 3);
					...
					foreach ( $products as $item ) {
                        ...
                        <li class="">
                            ...
                            <div class="product-item-right" data-row-id="<?php esc_attr_e( $item['prod_id'] ); ?>">
                                ...
                                <h4 class="product-title"><a href="<?php echo esc_url( $_product->get_permalink() ); ?>"><?php echo wp_specialchars_decode( $product_name ); ?></a>
                                </h4>
						...
						<?php if ( $with_tabs ) : ?>
                            <form method="post" autocomplete="off"
                                ...
                                <p class="woocommerce-FormRow">
                                    ...
                                    <input type="hidden" name="_wp_http_referer" value="<?php echo esc_url(wc_get_page_permalink( 'myaccount' )); ?>">
...
if ( ! function_exists( 'etheme_filter_widgets_classes' ) ) {
	function etheme_filter_widgets_classes( $params ) {
		if ( isset($params[0]['before_widget']) ) {
			$params[0]['before_widget'] = str_replace(array('sidebar-widget', 'topbar-widget',  'mobile-sidebar-widget',  'top-panel-widget',  'footer-widget',  'copyrights-widget' ), array(''), $params[0]['before_widget']);
		}
		return $params;
	}
}
...
//		}
		...
		if ( class_exists('YITH_WCBK_Frontend')) {
			...
			switch ( $booking_form_position ) {
				case 'before_summary':
				case 'after_summary':
					add_action( 'etheme_woocommerce_template_single_add_to_cart', array( 'YITH_WCBK_Frontend', 'print_add_to_cart_template' ) );
					break;
				case 'after_title':
					add_action( 'etheme_woocommerce_template_single_add_to_cart', array( 'YITH_WCBK_Frontend', 'print_add_to_cart_template' ), 7 );
					break;
				case 'before_description':
					add_action( 'etheme_woocommerce_template_single_add_to_cart', array( 'YITH_WCBK_Frontend', 'print_add_to_cart_template' ), 15 );
					break;
				case 'after_description':
					add_action( 'etheme_woocommerce_template_single_add_to_cart', array( 'YITH_WCBK_Frontend', 'print_add_to_cart_template' ), 25 );
					break;
			}
		}
...
if ( !function_exists('etheme_request_quote') ) {
	function etheme_request_quote() {
		...
		if (!$element_options['request_quote_settings']['request_quote'] ) return;
		...
		if ( $element_options['request_quote_settings']['popup_background_image'] != '' ) {
			$element_options['output_css'][] = 'background-image:' . $element_options['request_quote_settings']['popup_background_image'];
		...
		<?php if ( $element_options['request_quote_settings']['show_as_button'] ) : ?>
            <span class="inline-block pos-relative et-call-popup btn black" data-type="single-product-quote">
            <?php if ($element_options['ask_icon'] != '') echo '<span class="et_b-icon">' . $element_options['ask_icon'] . '</span>'; ?>
            <?php if ($element_options['request_quote_settings']['label'] != '' ) echo '<span>' . esc_html($element_options['request_quote_settings']['label']) . '</span>'; ?>
        </span>
		<?php else: ?>
...
//
				$product_reviews = ! in_array( 'reviews', (array) get_theme_mod( 'product_tabs_sortable', array('description','additional_information','reviews','et_custom_tab_01','et_custom_tab_02','single_custom_tab_01' ) ) ) && get_theme_mod( 'product_reviews_et-desktop', 0 );
