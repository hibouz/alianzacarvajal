...
class Instagram extends Widgets {
	...
	function widget( $args, $instance ) {
		...
		if ( ! $user ) {
			echo '<p class="woocommerce-info">' . esc_html__( 'To use this element select instagram user', 'xstore-core' ) . '</p>';
		} else {
...
//		$instance['spacing']  = ( $new_instance['spacing'] != '' ) ? true : false;
	...
	function et_get_instagram( $number = '', $tag = false, $last = '', $token = false, $type = 'widget', $tag_type = false ) {
		...
		if ( ! count( $api_data ) ) {
			return new \WP_Error( 'error', '<p class="woocommerce-info">' . esc_html__( 'To use this element select instagram user', 'xstore-core' ) . '</p>' );
		}
