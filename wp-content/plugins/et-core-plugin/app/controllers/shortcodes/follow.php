...
class Follow extends Shortcodes {
    ...
    function follow_shortcode( $atts ) {
        ...
        <div class="et-follow-buttons <?php echo implode(' ', $options['class']); ?>">
            ...
            <?php 
                foreach ( $options['socials'] as $social => $key ) {
                    if ( $atts[$social] != '' ) {
                        $icon = $social == 'email' ? 'message' : $social; ?>
                        <a href="<?php echo esc_url( $atts[$social] ); ?>" class="follow-<?php echo esc_attr($social); ?><?php echo ( '' != $atts['tooltip']) ? ' mtips' : ''; ?>" <?php echo $atts['target']; ?> rel="nofollow">
