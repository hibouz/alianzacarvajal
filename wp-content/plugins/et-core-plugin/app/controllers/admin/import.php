...
class Import extends Base_Controller {
	...
	public function init() {
		...
		$this->import_url  = 'http://8theme.com/import/xstore-demos/';
	...
	public function update_menus($multiple = false){
		...
		if ( wp_get_nav_menu_object( 'Mobile menu' ) ) {
			$menus['mobile_menu_term'] = wp_get_nav_menu_object( 'Main menu' );
		} else {
	...
	public function get_version_info(){
		...
		if( ! is_wp_error( $data ) && $code == '200' ) {
			...
			update_option('etheme_current_version',$data);
	...
	public function get_file_name($file){
		$files = array(
			'fonts'=> 'fonts.json',
			'brands'=> 'brands.json',
			'product_cats'=> 'product_cats.json',
