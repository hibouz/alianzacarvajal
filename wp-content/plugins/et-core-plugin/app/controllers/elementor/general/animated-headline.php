...
//
//	// if highlighted
//	Marker (type of animation effect)
//		Curly
//		Circle
//		Undeline Zigzag
//		X
...
//					'crossthrough' => __('Cross-through', 'xstore-core'), // cross-through
		...
		$this->add_control(
			'shape_custom',
			[
				'label' => esc_html__( 'Custom SVG icon shape', 'elementor' ),
...
//				],
		...
		$this->add_control(
			'animated_text_color',
			[
				'label' => __( 'Animated Text color', 'xstore-core' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'separator' => 'before',
				'selectors' => [
...
//	    if ( !\Elementor\Plugin::$instance->editor->is_edit_mode() ) {
		    if ( function_exists( 'etheme_enqueue_style' ) ) {
			    etheme_enqueue_style( 'etheme-animated-headline', true );
		    }
//	    }
        ...
        if ( $settings['text_style'] == 'animate' ) {
	        ...
	        $this->add_render_attribute( 'headline', 'data-animation-type', $settings['animation_type'] );

	        if ( in_array( $settings['animation_type'], [ 'typing', 'swirl', 'swirl-2', 'wave' ] ) ) {
		        $this->add_render_attribute( 'headline', 'class', 'etheme-headline-letters' );
	        }
        }
        
        else {
	        $this->add_render_attribute( 'headline', 'data-loop', $settings['loop'] );
	        $this->add_render_attribute( 'headline', 'data-anim-duration', ($settings['duration']['size'] != '' ? ((float)$settings['duration']['size']*1000) : 2400) );
	        $this->add_render_attribute( 'headline', 'data-anim-delay', (isset($settings['delay']['size']) && $settings['delay']['size'] != '' ? ((float)$settings['delay']['size']*1000) : 0) );
        }
        ...
        if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) { ?>
            <script>
                jQuery(document).ready(function() {
                    etTheme.ethemeAnimatedHeadline();
                });
            </script>
			<?php
		}
