...
class Tabs extends \Elementor\Widget_Base {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .et-advance-tabs .et-tabs-nav > ul li img, {{WRAPPER}} .et-advance-tabs .et-tabs-nav > ul li i svg' => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .et-advance-tabs .et-tabs-nav > ul li i svg' => 'height: {{SIZE}}{{UNIT}};',
				],
	...
	protected function render() {
		...
		if ( function_exists('etheme_enqueue_style')) {
			etheme_enqueue_style( 'et-advance-tabs', true );
		}
                        ...
                        <li class="<?php echo esc_attr($tab['et_tabs_tab_show_as_default']); ?> et-tab-nav"
							...
							<?php if ( 'yes' === $settings['et_tabs_icon_show_horizontal'] ):
								if ( $tab['et_tabs_icon_type'] === 'icon' ):
									...
									if ( $is_new || $migrated ) {
										?><i><?php \Elementor\Icons_Manager::render_icon( $tab['et_tabs_tab_title_icon'], [ 'aria-hidden' => 'true' ] );?></i><?php
						...
						switch ( $tab['et_tabs_text_type'] ) {
							...
							case 'template':
								if ( !empty( $tab['et_primary_templates'] ) ):
									echo \Elementor\Plugin::$instance->frontend->get_builder_content( $tab['et_primary_templates'], true );
								endif;
								break;
							case 'staticblocks':
