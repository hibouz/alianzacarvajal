...
if ( ! function_exists( 'et_ajax_element_holder' ) ) {
	function et_ajax_element_holder($element = false, $atts = array(), $extra = '', $content = false, $type = 'element', $widget = array()){
		...
		if ( $content ) {
			$content = '<span class="hidden et-element-content">' . $content . '</span>';
		}
		
		if (count($widget)){
			$content .= '<span class="hidden et-element-args_widget">' . esc_js(json_encode($widget, JSON_HEX_QUOT) ). '</span>';
		}
		...
		$output = '
			<div class="et-load-block lazy-loading et-ajax-element type-'.$type.'" data-type="'.$type.'" data-extra="' . $extra . '" data-element="' . $element . '">
				...
				<span class="hidden et-element-args">' . json_encode( $atts ) . '</span>
				' . $content . '
			</div>
		';
		return $output;
