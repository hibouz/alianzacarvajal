...
//                    $etheme_single_product_variation_gallery = etheme_get_option('enable_variation_gallery');
			...
			if ( is_product() ) {
				...
				if ($single_product_widget_area_1_widget_toggle_actions == 'closed' || (($single_product_widget_area_1_widget_toggle_actions == 'closed_mobile') && $is_mobile_device) ) {
					set_query_var('et_sidebar-widgets-open-close-default', true);
                }
			}
	...
	function etheme_add_body_classes($classes) {
		...
		if ( get_query_var('et_is-woocommerce', false)) {
			...
			if ( get_query_var('is_single_product', false) ) {
				...
				if ( !$etheme_single_product_builder ) {
					$classes[] = 'global-product-name-'.(etheme_get_option('product_name_signle', 0) ? 'off': 'on');
				}
			}
		}
...
//							break;
		...
		if( ! empty( $bread_bg['background-image'] ) || ! empty( $bread_bg['background-color'] ) ){
			...
			if ( (get_query_var('et_is-cart', false) || get_query_var('et_is-checkout', false)) &&
                 etheme_get_option( 'cart_special_breadcrumbs', 1 ) ) {
	...
	function etheme_get_image($attach_id, $size, $location = '') {
		...
		if ( $type && $type != 'default' ) {
			if ( $type == 'lqip') {
				...
				$new_attr = 'src="' . etheme_placeholder_image($size, $attach_id) . '" data-src';
				$image    = str_replace( 'src', $new_attr, $image );
			}
...
//	}
    ...
    <?php if ( apply_filters('etheme_preload_woff_icons', true)) : ?>
        <link rel="preload" as="font" href="<?php echo esc_url( get_template_directory_uri() ); ?>/fonts/xstore-icons-<?php echo esc_attr($icons_type); ?>.woff?v=<?php echo esc_attr( ETHEME_THEME_VERSION ); ?>" type="font/woff" crossorigin>
    <?php endif; ?>

	<?php if ( apply_filters('etheme_preload_woff2_icons', true)) : ?>
        <link rel="preload" as="font" href="<?php echo esc_url( get_template_directory_uri() ); ?>/fonts/xstore-icons-<?php echo esc_attr($icons_type); ?>.woff2?v=<?php echo esc_attr( ETHEME_THEME_VERSION ); ?>" type="font/woff2" crossorigin>
    <?php
    endif;
});
